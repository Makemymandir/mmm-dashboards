...
async function loadProject(projectId) {
  ...
  if (cached) {
    ...
    container.innerHTML = '<div style="padding:80px;text-align:center;"><div style="font-size:2.5rem;margin-bottom:16px;">🛕</div><div style="color:var(--grey);font-size:1rem;">Loading project...</div></div>';
  }
  ...
  try {
    ...
    console.error(err);
    if (!currentProject) showError('Connection error. Please refresh and try again.');
  }
}
...
async function loadRoughEstimates() {
  ...
  if (cached && roughEstimates.length === 0) {
    ...
    container.innerHTML = '<div style="padding:40px;text-align:center;color:var(--grey);">Loading estimates...</div>';
  }

  try {
    const result = await api.call('list_rough_estimates', { project_id: currentProject.project_id });
    if (!result.ok) {
      if (roughEstimates.length === 0) container.innerHTML = '<div class="empty-tab"><h3>Error</h3><p>' + escapeHtml(result.error) + '</p></div>';
      return;
    }
...
function renderRoughList() {
  ...
  if (roughEstimates.length === 0) {
    container.innerHTML = '<div class="empty-tab"><h3>No rough estimates yet</h3><p>Generate a quick price comparison across all materials.</p><button class="btn-primary" onclick="newRoughEstimate()">+ New Rough Estimate</button></div>';
    return;
  }
...
async function loadQuotations() {
  ...
  container.innerHTML = '<div style="padding:40px;text-align:center;color:var(--grey);">Loading quotations...</div>';
...
function renderQuotationsList() {
  ...
  quotations.forEach(function(q) {
    var statusClass = statusColors[q.status] || 'status-Lead';
...
function showError(msg) {
  document.getElementById('projectContent').innerHTML =
    '<div class="empty-state"><h3>' + escapeHtml(msg) + '</h3><p><a href="dashboard.html" class="btn-primary" style="display:inline-block;margin-top:12px;">← Back to Dashboard</a></p></div>';
}
  ...
  return `
    <div class="card" style="padding:0;overflow:hidden;">
      ...
      <div style="padding:24px 28px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:0;border:1px solid #F0E6DC;border-radius:10px;overflow:hidden;">
      ...
      <div style="padding:0 28px 24px;">
        ...
        <div style="background:#FFF7F0;border-left:3px solid var(--orange);border-radius:0 8px 8px 0;padding:14px 18px;font-size:0.9rem;line-height:1.6;white-space:pre-wrap;">${escapeHtml(p.notes)}</div>
      </div>` : ''}
...
function brandDetailRow(icon, label, value) {
  ...
  var isEmpty = (!value);
  return '<div style="display:flex;align-items:center;gap:12px;padding:13px 18px;border-bottom:1px solid #F0E6DC;background:' + (isEmpty ? '#FAFAFA' : '#fff') + ';">'
