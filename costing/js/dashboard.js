...
// Stage configuration — maps to Discover/Design/Deliver
const STAGES = {
  // DISCOVER
...
async function loadProjects() {
  ...
  try {
    ...
    if (!result.ok) {
      if (!allProjects.length) container.innerHTML = '<div class="empty-state"><h3>Error loading projects</h3><p>' + escapeHtml(result.error || 'Unknown error') + '</p></div>';
      return;
    }
    ...
    console.error(err);
    if (!allProjects.length) container.innerHTML = '<div class="empty-state"><h3>Connection error</h3><p>Please refresh.</p></div>';
  }
}
...
function renderStageSummary(projects) {
  ...
  phases.forEach(function(phase) {
    ...
    if (count === 0) return;
    html += '<div style="display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:20px;background:' + phase.bg + ';cursor:pointer;" onclick="filterByPhase(\'' + phase.key + '\')">';
  ...
  html += '<div style="display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:20px;background:#F5F5F5;cursor:pointer;" onclick="clearPhaseFilter()">';
