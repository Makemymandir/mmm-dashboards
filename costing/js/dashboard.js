...
function getStageStyle(status) {
  return STAGES[status] || { color: '#757575', bg: '#F5F5F5', phase: 'OTHER' };
}
...
async function loadProjects() {
  ...
  if (cached) {
    ...
    } catch (e) { sessionStorage.removeItem('mmm_all_projects'); }
  } else {
    container.innerHTML = '<div style="padding:60px;text-align:center;"><div style="font-size:2rem;margin-bottom:12px;">🛕</div><div style="color:var(--grey);">Loading projects...</div></div>';
  }

  try {
    var result = await api.call('list_projects', {});
    if (!result.ok) {
      if (!allProjects.length) container.innerHTML = '<div class="empty-state"><h3>Error loading projects</h3><p>' + escapeHtml(result.error || '') + '</p></div>';
      return;
    }
    ...
    console.error(err);
    if (!allProjects.length) container.innerHTML = '<div class="empty-state"><h3>Connection error</h3><p>Please refresh and try again.</p></div>';
  }
}
...
function renderStageSummary(projects) {
  ...
  projects.forEach(function(p) {
    var phase = getStageStyle(p.status).phase;
    counts[phase] = (counts[phase] || 0) + 1;
  });

  var phases = [
    { key: 'DISCOVER', label: 'Discover', color: '#1565C0', bg: '#E3F2FD' },
    { key: 'DESIGN',   label: 'Design',   color: '#E65100', bg: '#FFF3E0' },
    { key: 'DELIVER',  label: 'Deliver',  color: '#2E7D32', bg: '#E8F5E9' },
    { key: 'OTHER',    label: 'Other',    color: '#757575', bg: '#F5F5F5' }
  ];

  var html = '';
  phases.forEach(function(phase) {
    var count = counts[phase.key] || 0;
    if (count === 0) return;
    html += '<div onclick="filterByPhase(\'' + phase.key + '\')" style="display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:20px;background:' + phase.bg + ';cursor:pointer;">';
    html += '<span style="font-size:0.85rem;font-weight:700;color:' + phase.color + ';">' + phase.label + '</span>';
    html += '<span style="background:' + phase.color + ';color:white;border-radius:10px;padding:1px 7px;font-size:0.75rem;font-weight:700;">' + count + '</span>';
    html += '</div>';
  });
  html += '<div onclick="clearPhaseFilter()" style="display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:20px;background:#F5F5F5;cursor:pointer;">';
  html += '<span style="font-size:0.85rem;color:#757575;">All (' + projects.length + ')</span>';
  html += '</div>';
...
function filterProjects() {
  ...
  var filtered = allProjects.filter(function(p) {
    ...
    var matchPhase  = !currentPhaseFilter || getStageStyle(p.status).phase === currentPhaseFilter;
...
function renderProjects(projects) {
  ...
  if (projects.length === 0) {
    container.innerHTML = '<div class="empty-state"><h3>No projects found</h3><p>Try a different search or filter.</p><button class="btn-primary" onclick="openNewProjectModal()" style="margin-top:16px;">+ New Project</button></div>';
    return;
  }

  var html = '<table class="project-table"><thead><tr>';
  html += '<th>Project ID</th><th>Client</th><th>City</th><th>Framework</th>';
  html += '<th>Stage</th><th>Design Fee</th><th>Created</th><th></th>';
  html += '</tr></thead><tbody>';

  projects.forEach(function(p) {
    var stage     = getStageStyle(p.status);
    var feeStatus = p.design_fee_status || '';
    var feeBadge  = '';

    if (feeStatus === 'Paid') {
      feeBadge = '<span style="background:#E8F5E9;color:#2E7D32;padding:2px 8px;border-radius:10px;font-size:0.75rem;font-weight:600;">Paid</span>';
    } else if (feeStatus === 'Redeemable Used') {
      feeBadge = '<span style="background:#F3E5F5;color:#6A1B9A;padding:2px 8px;border-radius:10px;font-size:0.75rem;font-weight:600;">Redeemed</span>';
    } else {
      feeBadge = '<span style="background:#FFEBEE;color:#C62828;padding:2px 8px;border-radius:10px;font-size:0.75rem;font-weight:600;">Not Paid</span>';
    }
    ...
    html += '<td><span class="status-badge" style="background:' + stage.bg + ';color:' + stage.color + ';border:none;">' + escapeHtml(p.status || 'Inquiry') + '</span></td>';
    html += '<td>' + feeBadge + '</td>';
...
function openNewProjectModal() {
  ...
  ['np_client_name','np_contact','np_email','np_location','np_consultation_notes'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.value = '';
  });
  ['np_framework','np_budget','np_timeline','np_customer_type','np_design_fee_status','np_source'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.selectedIndex = 0;
  });
  ['np_width','np_depth','np_height'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.value = '';
  });

  setTimeout(function() {
    var el = document.getElementById('np_client_name');
    if (el) el.focus();
  }, 100);
}
...
async function createProject() {
  ...
  var widthEl  = document.getElementById('np_width');
  var depthEl  = document.getElementById('np_depth');
  var heightEl = document.getElementById('np_height');
  ...
  var project = {
    ...
    location:            location,
    framework:           framework,
    width_ft:            widthEl.value  !== '' ? parseFloat(widthEl.value)  : '',
    depth_ft:            depthEl.value  !== '' ? parseFloat(depthEl.value)  : '',
    height_ft:           heightEl.value !== '' ? parseFloat(heightEl.value) : '',
  ...
  try {
    ...
    if (result.ok) {
      toast('Project created: ' + result.project_id, 'success');
