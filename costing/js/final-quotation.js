...
function renderSectionLines(sectionLines, lineType) {
  ...
  if (lineType === 'material') html += '<th style="text-align:left;padding:8px 6px;color:var(--grey);font-size:0.8rem;">Supplier</th>';
  ...
  sectionLines.forEach(function(line) {
    ...
    if (lineType === 'material') html += '<td style="padding:10px 6px;color:var(--grey);font-size:0.85rem;">' + escapeHtml(line.supplier_id_used || '—') + '</td>';
    ...
    html += '<td style="padding:10px 6px;text-align:right;"><button onclick="deleteLine(\'' + line.line_id + '\')" style="background:none;border:none;color:#E53935;cursor:pointer;font-size:1rem;">✕</button></td>';
    html += '</tr>';
  });
  html += '</tbody></table>';
  return html;
...
function renderTotalsCard() {
  ...
  html += '<p class="totals-internal-note">⚠️ Internal view only. Client PDF distributes profit proportionally.</p>';
  html += '</div>';
  return html;
}
...
function filterMaterials() {
  ...
  matches.forEach(function(m) {
    var dr = m.default_rate;
    var drText = dr ? (isNaN(parseFloat(dr.cost_per_unit)) ? 'On Request' : '₹' + dr.cost_per_unit + '/' + (m.unit || 'sqft')) : 'No rate';
    ...
    html += '<span style="float:right;color:var(--orange);font-size:0.85rem;">' + drText + '</span>';
    html += '</div>';
  });
...
function selectMaterial(materialId) {
  ...
  suppRates.forEach(function(rate) {
    ...
    var costDisplay = (costVal === 'on request' || isNaN(parseFloat(costVal))) ? 'On Request' : '₹' + costVal + '/' + (selectedMaterial.unit || 'sqft');
  ...
  if (selectedMaterial.default_rate) {
    var dc = selectedMaterial.default_rate.cost_per_unit;
    if (dc !== 'on request' && !isNaN(parseFloat(dc))) {
      document.getElementById('lineRate').value = dc;
      calcLineTotal();
    }
  }
...
async function submitAddLine() {
  ...
  if (section === 'material') {
    if (!selectedMaterial) { errorEl.textContent = 'Please select a material.'; errorEl.style.display = 'block'; return; }
    ...
    if (!selectedMaster) { errorEl.textContent = 'Please select an item from the list.'; errorEl.style.display = 'block'; return; }
    var nameKey = window._masterNameKey || 'item_name';
  ...
  try {
    var result = await api.call('add_quotation_line', {
      quotation_id: quotation.quotation_id, line_type: section,
      ref_master_id: refMasterId, description: description,
      supplier_id_used: supplierName, qty: qty, unit: unit,
      cost_per_unit: rate, notes: notes, username: user.username
    });
    if (result.ok) {
...
async function deleteLine(lineId) {
  ...
  try {
    var result = await api.call('delete_quotation_line', { line_id: lineId, quotation_id: quotation.quotation_id });
    if (result.ok) {
      var qResult = await api.call('get_quotation', { quotation_id: quotation.quotation_id });
      if (qResult.ok) { quotation = qResult.quotation; lines = qResult.lines || []; }
    ...
    } else { toast('Failed to remove line', 'error'); }
...
async function updateQuotationField(field, value) {
  try {
    ...
    var result = await api.call('update_quotation', { quotation_id: quotation.quotation_id, updates: updates });
...
async function updateProfitPct(value) {
  ...
  try {
    var result = await api.call('update_quotation', { quotation_id: quotation.quotation_id, updates: { profit_pct: pct } });
    if (result.ok) {
      var qResult = await api.call('get_quotation', { quotation_id: quotation.quotation_id });
...
async function generatePdf() {
  ...
  var btn = document.querySelector('button.btn-primary[onclick="generatePdf()"]');
  ...
  try {
    ...
    p1div.innerHTML = buildQuotationPageHtml();
    ...
    p2div.innerHTML = buildProcessPageHtml();
    root.appendChild(p2div);
    await waitForPdfImages(root);

    pdf.addPage('a4', 'portrait');
    var canvas2 = await html2canvas(p2div.firstElementChild, {
      scale: 2, useCORS: true, allowTaint: true, backgroundColor: '#ffffff', logging: false
    });
...
function buildQuotationPageHtml() {
  ...
  var q = quotation;
  var frameworkIcons = {
    'Sinhasan':'framework-sinhasan.png','Wall/Floor Mounted':'framework-wfm.png',
    'Back Panel':'framework-bpn.png','Open Cabinet':'framework-ocm.png',
    'Cabinet with Door':'framework-cmd.png','Mandir Room':'framework-mdr.png'
  };
  var frameworkIcon = frameworkIcons[p.framework] || 'logo.png';
  var today      = new Date().toLocaleDateString('en-IN', { day:'2-digit', month:'2-digit', year:'numeric' });
  var validUntil = q.valid_until ? new Date(q.valid_until).toLocaleDateString('en-IN', { day:'2-digit', month:'2-digit', year:'numeric' }) : '—';
...
function formatDateInput(val) {
  if (!val) return '';
  try { var d = new Date(val); if (isNaN(d.getTime())) return ''; return d.toISOString().split('T')[0]; }
  catch (e) { return ''; }
}
